odoo.define('bi_pos_return_order.ReturnOrderButton', function(require) {
	'use strict';
	...
	ProductScreen.addControlButton({
		...
		condition: function() {
			return this.env.pos.config.show_order && this.env.pos.config.is_enabled_return;
		},
	});
	Registries.Component.add(ReturnOrderButton);
	const RefundButton = Registries.Component.get('RefundButton');
	ProductScreen.addControlButton({
		component: RefundButton,
        condition: function(){return this.env.pos.config.is_enabled_refund},
        position: ['replace', 'RefundButton'],
	});
	const SeePOSOrdersButton = Registries.Component.get('SeePOSOrdersButton');
	ProductScreen.addControlButton({
		component: SeePOSOrdersButton,
        condition: function(){return this.env.pos.config.show_order && this.env.pos.config.is_hide_order_button},
        position: ['replace', 'SeePOSOrdersButton'],
	});
