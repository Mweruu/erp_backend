odoo.define('dk_accounting_tremol_integration.action_post_send_invoice', function (require) {
        ...
        for (let move_id in params.invoices) {
            try {
                let res = await rpc.query({
                    model: 'tremol.queue',
                    method: 'queue_tremol',
                    args: [params.invoices[move_id].proxy_address  + '/hw_proxy/l10n_ke_cu_send',
                    params.invoices[move_id].messages,params.invoices[move_id].company_vat]
                });
