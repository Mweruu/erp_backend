...
class ResConfigSettings(models.TransientModel):
    ...
    so_double_validation = fields.Selection(
        ...
        string="Levels of Approvals in Sales ",
        default='one_step',
        help="Provide a double validation mechanism for purchases"
    )

    so_double_validation_amount = fields.Monetary(
        string='Sales Double validation amount',
        default=5000,
        help="Minimum amount for which a double validation is required"
    )
    sale_set_percentage_range = fields.Float(string='Sale Percentage Range (%)',
                                             config_parameter="dk_sale_automatic_workflow.sale_set_percentage_range",
                                             )
    sale_set_percentage_range_bool = fields.Boolean(string="Sale Percentage Range",
                                                    config_parameter="dk_sale_automatic_workflow.sale_set_percentage_range_bool",
                                                    )
