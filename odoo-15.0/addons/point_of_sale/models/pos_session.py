...
class PosSession(models.Model):
    ...
    cash_register_id = fields.Many2one('account.bank.statement', compute='_compute_cash_all', string='Cash Register',
                                       store=True)
        ...
        for session in self:
            ...
            if cash_payment_method:
                ...
                result = self.env['pos.payment'].read_group(
                    [('session_id', '=', session.id), ('payment_method_id', '=', cash_payment_method.id)], ['amount'],
                    ['session_id'])
                if result:
    ...
    def _check_bank_statement_state(self):
        for session in self:
            ...
            if closed_statement_ids:
                raise UserError(
                    _("Some Cash Registers are already posted. Please reset them to new in order to close the session.\n"
    ...
    def _get_balancing_account(self):
        ...
        return self.company_id.account_default_pos_receivable_account_id or propoerty_account or self.env[
            'account.account']
            ...
            if not order_is_invoiced:
                ...
                if self.company_id.anglo_saxon_accounting and order.picking_ids.ids:
                    ...
                    for move in stock_moves:
                        ...
                        amount = signed_product_qty * move.product_id._compute_average_price(0, move.quantity_done,
                                                                                             move)
    ...
    def _create_cash_statement_lines_and_cash_move_lines(self, data):
        ...
        for payment, amounts in split_receivables_cash.items():
            ...
            split_cash_statement_line_vals[statement].append(
                self._get_split_statement_line_vals(statement, amounts['amount'], payment))
        ...
        for payment_method, amounts in combine_receivables_cash.items():
            if not float_is_zero(amounts['amount'], precision_rounding=self.currency_id.rounding):
                ...
                combine_cash_statement_line_vals[statement].append(
                    self._get_combine_statement_line_vals(statement, amounts['amount'], payment_method))
        ...
        for statement in self.statement_ids:
            split_cash_statement_lines[statement] = BankStatementLine.create(
                split_cash_statement_line_vals[statement]).mapped('move_id.line_ids').filtered(
                lambda line: line.account_id.internal_type == 'receivable')
            combine_cash_statement_lines[statement] = BankStatementLine.create(
                combine_cash_statement_line_vals[statement]).mapped('move_id.line_ids').filtered(
                lambda line: line.account_id.internal_type == 'receivable')
    ...
    def _reconcile_account_move_lines(self, data):
        ...
        for statement in self.statement_ids:
            ...
            lines_by_account = [all_lines.filtered(lambda l: l.account_id == account and not l.reconciled) for account
                                in accounts if account.reconcile]
