...
class CrmClaimEpt(models.Model):
    ...
    state = fields.Selection([
        ('draft', 'Draft'), ('waiting_for_approval', 'Waiting for Approval'), ('approve', 'Approved'),
        ('process', 'Processing'), ('close', 'Closed'), ('reject', 'Rejected')], default='draft', copy=False,
        tracking=True)
    ...
    new_sale_id = fields.Many2one(SALE_ORDER, string='New Sale Order', copy=False)

    location_id = fields.Many2one('stock.location', string='Return Location',
                                  domain=[('usage', '=', 'internal'), ('return_location', '=', True)])
    internal_picking_id = fields.Many2one(STOCK_PICKING, string='Internal Delivery Order', copy=False)
    picking_id = fields.Many2one(STOCK_PICKING, string='Delivery Order')
    return_picking_id = fields.Many2one(STOCK_PICKING, string='Return Delivery Order', copy=False)
    rma_support_person_id = fields.Many2one(RES_PARTNER, string='Contact Person')
    partner_delivery_id = fields.Many2one(RES_PARTNER, string='Delivery Address')
    ...
    refund_invoice_ids = fields.Many2many(ACCOUNT_MOVE, string='Refund Invoices', copy=False)
    ...
    def onchange_is_rma_without_incoming(self):
        ...
        warning = False
        repair_lines = self.claim_line_ids.filtered(lambda l: l.rma_reason_id and l.rma_reason_id.action == 'repair')
    ...
    def create(self, vals_list):
        """Need to add claim's Partner and Responsible person into followers"""
        context = dict(self._context or {})
        ...
        # Create records with updated vals_list
        result = super().create(vals_list)

        # Process each created record
        for record in result:
            self.add_email_in_partner(record)
            partner_id = record.rma_support_person_id.id if record.rma_support_person_id else record.partner_id.id
            self.sudo().add_followers_in_claim(record, partner_id)
        ...
        for claim_line in self.claim_line_ids:
            ...
            if claim_line.serial_lot_ids:
                ...
                update_number_lines = self.return_picking_id.move_ids.mapped('move_line_ids').filtered( \
                    lambda l,
                           claim_line=claim_line: l.product_id.id == claim_line.product_id.id and l.lot_id.id not in claim_line.serial_lot_ids.ids and l.qty_done == 0.0)

                # process an return move lines
                return_move_lines = self.return_picking_id.move_ids.mapped('move_line_ids').filtered( \
                    lambda l, claim_line=claim_line: l.product_id.id == claim_line.product_id.id and l.qty_done == 0.0)
                for serial_lot_id in claim_line.serial_lot_ids:
                    return_lot_move_lines = return_move_lines.filtered(
                        lambda l, serial_lot_id=serial_lot_id: l.lot_id.id == serial_lot_id.id and l.qty_done == 0.0)
                    if not return_lot_move_lines and update_number_lines:
                        update_number_lines = update_number_lines.filtered(lambda l: l.qty_done == 0.0)
    ...
    def check_claim_line_validate(line):
        ...
        if line.product_id.tracking == 'lot':
            if len(line.serial_lot_ids) == 0:
                raise UserError(_("Please set Lot number for the product: '%s'.") % (line.product_id.name))
        ...
        for line in claim_lines:
            ...
            if line.product_id.tracking == 'serial':
                for lot_id in line.serial_lot_ids:
                    ...
                    repair_order_dict.update({'lot_id': lot_id.id, 'sale_order_id': sale_order_id})
    ...
    def create_do(self, claim_lines):
        ...
        pickings = self.env[STOCK_PICKING].search([('group_id', '=', group_id.id)])
