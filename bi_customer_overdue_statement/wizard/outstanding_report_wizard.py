    ...
    def print_outstanding_report(self):
        ...
        for journal_id in unique_journal_records:
            self._cr.execute(
                ...
                (journal_id['id'], self.start_date, self.end_date))
            uniq_partner_records = self._cr.dictfetchall()
            final_group_data = {}
            for partner in uniq_partner_records:
                self._cr.execute(
                    "select * from account_move where invoice_date_due between (%s) and (%s) and (journal_id=%s) and partner_id = (%s) and state != 'paid' order by invoice_date_due ASC",
                    (self.start_date, self.end_date, journal_id['id'], partner['partner_id']))
        ...
        if list_of_month:
            ...
            for key1, data1 in last_group_data.items():
                ...
                for key, value in od.items():
                    self._cr.execute(
                        "select sum(amount_residual),invoice_date_due  from account_move where Extract(month from invoice_date_due)=(%s) and journal_id=(%s) group by invoice_date_due",
                        (value, key1))
        ...
        if self._context.get('type') == 'pdf':
            ...
            return self.env.ref('bi_customer_overdue_statement.report_outstanding_print').report_action(self,
                                                                                                        data=datas)
        ...
        export_id = self.env['outstanding.report.excel'].create(
