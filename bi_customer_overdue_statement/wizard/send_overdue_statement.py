...
class account_report_partner_ledger(models.TransientModel):
    ...
    confirm_text = fields.Char(default="Press Send Overdue Payment to send email notification to customer",
                               readonly=True)

    def send_overdue_statement_customer(self):
        res = self.env['res.partner'].browse(self._context.get('active_ids', []))
        for user in res:
            user.customer_do_partner_mail()
...
class customer_statement_wizard(models.TransientModel):
    ...
    confirm_text = fields.Char(default="Press Send Customer Statement to send email notification to customer",
                               readonly=True)
