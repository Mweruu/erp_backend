...
class Res_Partner(models.Model):
    ...
    def _get_amounts_and_date_amount(self):
        ...
        for partner in self:
            ...
            if partner.monthly_statement_line_ids:
                ...
                partner.sup_monthly_payment_amount_due_amt = 0.0
                partner.sup_monthly_payment_amount_overdue_amt = 0.0
            ...
            if partner.weekly_statement_line_ids:
                ...
                partner.sup_weekly_payment_amount_due_amt = 00
                partner.sup_weekly_payment_amount_overdue_amt = 00
            ...
            for mv in moves:
                for ml in mv.line_ids:
                    if ml.account_id.account_type == 'asset_receivable':
                        ...
                        if partner.custom_from_date and partner.custom_to_date and ml.date_maturity:
                            if ml.date_maturity >= partner.custom_from_date and ml.date_maturity <= partner.custom_to_date:
                                if ml.account_id.account_type == 'asset_receivable':
                                    # if ml.account_id.internal_type =='receivable':
                                    if ml.date_maturity:
                                        diff = today - ml.date_maturity
    ...
    invoice_ids = fields.Many2many('account.move', 'invoice_id', string='Invoice Ids')
    ...
    filter_payment_amount_overdue_amt = fields.Float(compute='_get_amounts_and_date_amount',
                                                     string="Filter Total Overdue Amount", compute_sudo=True)
    monthly_payment_amount_due_amt = fields.Float(compute='_get_amounts_and_date_amount',
                                                  string=" Filter Monthly Balance Due", compute_sudo=True)
    monthly_payment_amount_overdue_amt = fields.Float(compute='_get_amounts_and_date_amount',
                                                      string="Filter Total Monthly Overdue Amount", compute_sudo=True)

    weekly_payment_amount_due_amt = fields.Float(compute='_get_amounts_and_date_amount',
                                                 string="Filter Weekly Balance Due", compute_sudo=True)
    weekly_payment_amount_overdue_amt = fields.Float(compute='_get_amounts_and_date_amount',
                                                     string="Filter Weekly Total Overdue Amount", compute_sudo=True)

    sup_monthly_payment_amount_due_amt = fields.Float(compute='_get_amounts_and_date_amount',
                                                      string=" Filter Supplier Monthly Balance Due", compute_sudo=True)
    sup_monthly_payment_amount_overdue_amt = fields.Float(compute='_get_amounts_and_date_amount', compute_sudo=True,
    ...
    sup_weekly_payment_amount_due_amt = fields.Float(compute='_get_amounts_and_date_amount',
                                                     string="Filter Supplier Weekly Balance Due", compute_sudo=True)
    sup_weekly_payment_amount_overdue_amt = fields.Float(compute='_get_amounts_and_date_amount', compute_sudo=True,
                                                         string="Filter Supplier Weekly Total Overdue Amount")

    filter_payment_amount_due_amt_supplier = fields.Float(compute='_get_amounts_and_date_amount', compute_sudo=True,
                                                          string="Filter Supplier Balance Due")
    filter_payment_amount_overdue_amt_supplier = fields.Float(compute='_get_amounts_and_date_amount', compute_sudo=True,
    ...
    def _cron_send_overdue_statement(self):
        ...
        if company.send_overdue_statement:
            partners.customer_do_partner_mail()
        return True
            ...
            if partner.payment_amount_due_amt == 0.00:
                ...
                if partner.email:
                    template = self.env.ref(
                        'bi_customer_overdue_statement.email_template_customer_due_statement')

                    report = self.env.ref('bi_customer_overdue_statement.report_customer_due_print')
                    ...
                    if report.report_type in ['qweb-html', 'qweb-pdf']:
                        result, report_format = report._render_qweb_pdf(report.report_name, res_ids=[partner.id])
                    else:
                        res = report._render_qweb_pdf(report.report_name, res_ids=[partner.id])
                        if not res:
    ...
    def do_due_supplier_partner_mail(self):
        ...
        for partner in self:
            ...
            if partner.sudo().payment_amount_due_amt_supplier == 0.00:
                ...
                if partner.email:
                    template = self.env.ref(
                        'bi_customer_overdue_statement.email_template_supplier_due_statement')
                    report = self.env.ref('bi_customer_overdue_statement.report_supplier_due_print')
                    ...
                    if report.report_type in ['qweb-html', 'qweb-pdf']:
                        result, report_format = report._render_qweb_pdf(report.report_name, [partner.id])
                    else:
                        res = report._render_qweb_pdf(report.report_name, res_ids=[partner.id])
                        if not res:
                            raise UserError(_('Unsupported report type %s found.', report.report_type))
    ...
    def customer_do_partner_mail(self):
        unknown_mails = 0
        for partner in self:
            ...
            if partner.payment_amount_overdue_amt == 0.00:
                ...
                if partner.email:
                    ...
                    if not template:
                        template = self.env.ref(
                            'bi_customer_overdue_statement.email_template_customer_over_due_statement')
        ...
        for partner in self:
            ...
            if partner.payment_amount_overdue_amt_supplier == 0.00:
                ...
                if partner.email:
                    ...
                    if not template:
                        template = self.env.ref(
                            'bi_customer_overdue_statement.email_template_supplier_over_due_statement')

                    report = self.env.ref('bi_customer_overdue_statement.report_supplier_overdue_print')
    ...
    def supplier_monthly_send_mail(self):
        unknown_mails = 0
        for partner in self:
            partner.sup_monthly_payment_amount_due_amt = None
            partner._get_amounts_and_date_amount()
            if partner.opt_statement == False:
                if partner.sup_monthly_payment_amount_due_amt == 0.00:
                    pass
                else:
                    if partner.email:
                        template = self.env.user.company_id.sup_monthly_template_id

                        report = self.env.ref('bi_customer_overdue_statement.report_supplier_monthly_print')
    ...
    def supplier_weekly_send_mail(self):
        unknown_mails = 0
        for partner in self:
            partner.sup_weekly_payment_amount_due_amt = None
            partner._get_amounts_and_date_amount()
            if partner.opt_statement == False:
                if partner.sup_weekly_payment_amount_due_amt == 0.00:
                    pass
                else:
                    if partner.email:
                        template = self.env.user.company_id.sup_weekly_template_id

                        report = self.env.ref('bi_customer_overdue_statement.report_supplier_weekly_print')
    ...
    def supplier_send_mail(self):
        ...
        for partner in self:
            ...
            if partners_to_email:
                for pe in partners_to_email:
                    mail_template_id = self.env.ref('bi_customer_overdue_statement.email_template_supplier_statement')
                    mail_template_id.send_mail(pe.id)

                if partner not in partners_to_email:
                    msg = _('Supplier Statement email sent to %s' % ', '.join(
                        ['%s <%s>' % (partner.name, partner.email) for partner in partners_to_email]))
                    partner.message_post(body=msg)
        return unknown_mails

    def do_button_due_print(self):
        return self.env.ref('bi_customer_overdue_statement.report_customer_due_print').report_action(self)

    def do_button_supplier_due_print(self):
        return self.env.ref('bi_customer_overdue_statement.report_supplier_due_print').report_action(self)

    def customer_do_button_print(self):
    ...
    def do_supplier_button_print(self):
        return self.env.ref('bi_customer_overdue_statement.report_supplier_overdue_print').report_action(self)
    ...
    def do_customer_statement_mail(self):
        ...
        for partner in self:
            if partner.email:
                ...
                mail_template_id = self.env.ref(
                    'bi_customer_overdue_statement.email_template_customer_statement_custom_')
