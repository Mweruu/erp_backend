DROP FUNCTION if EXISTS public.si_ledger_get_products_movements_for_inventory_ledger_cmpwise(integer[], integer[], integer[], date, date, integer);
...
Insert into setu_inventory_ledger_bi_report(company_id, product_id, product_category_id, inventory_date,
    opening_stock, sales, purchase, sales_return, purchase_return, adjustment_in, adjustment_out, production_in, production_out, closing, wizard_id, total_in, total_out)
select 
d.company_id, d.product_id, d.product_category_id, d.inventory_date,
    ...
    d.purchase + d.sales_return + d.adjustment_in + d.production_in as total_in,
    d.purchase_return + d.sales + d.adjustment_out + d.production_out as total_out
from (
select
