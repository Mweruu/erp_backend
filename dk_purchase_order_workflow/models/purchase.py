...
class PurchaseOrder(models.Model):
    ...
    def button_confirm(self):
        ...
        template_id = self.env.ref("dk_purchase_order_workflow.email_template_for_approve_purchase_order")
    ...
    def action_approve_order(self):
        ...
        if next_line:
            ...
            template_id = self.env.ref(
                "dk_purchase_order_workflow.email_template_for_confirm_purchase_order")
            if template_id:
                template_id.sudo().send_mail(self.id, force_send=True, email_values={
    ...
    def button_approve(self, force=False):
        if self.ywt_purchase_order_automation_id:
            ...
            return self.action_purchase_order_automation_method()
        else:
        ...
        for order in self.filtered(lambda po: po.state in ('purchase', 'done')):
            ...
            for line in order.order_line.filtered(lambda l: l.product_id.type in ['product', 'consu']):
                pickings = order.picking_ids.filtered(
                    lambda x: x.state not in ('done', 'cancel') and x.picking_type_id.id == line.picking_type_id.id)
                if not pickings:
