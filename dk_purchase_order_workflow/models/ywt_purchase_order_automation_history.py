...
class YWTPurchaseOrderAutomationHistory(models.Model):
    ...
    mismatch_type = fields.Selection([('purchase', 'Purchase'), ('transfer', 'Transfer'), ('invoice', 'Invoice')],
                                     string="Mismatch Type")
    error_message = fields.Text(string="Error Message")

    def create_purchase_order_automation_history(self):
        sequence_id = self.env.ref('dk_purchase_order_workflow.ywt_purchase_automation_history_sequence').ids
        if sequence_id:
