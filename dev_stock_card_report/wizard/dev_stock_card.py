...
class dev_stock_card(models.TransientModel):
    ...
    def in_lines(self, product_ids):
        ...
        state = ('draft', 'cancel', 'waiting', 'confirmed', 'assigned')
        query = """select DATE(sm.date) as date, sm.origin as origin, sm.reference as ref, pt.name->>'en_US' as product,\
