...
class TestPayslipFlow(TestPayslipBase):
    ...
    def test_00_payslip_flow(self):
        ...
        context = {'model': 'hr.contribution.register',
                   'active_ids': [self.ref('om_om_hr_payroll.hr_houserent_register')]}
        test_reports.try_report_action(self.env.cr, self.env.uid, 'action_payslip_lines_contribution_register',
                                       context=context, our_module='om_hr_payroll')
